...
import { ArrowLeft, MapPin, Phone, Building2, Layers, CheckCircle, FileText, Image as ImageIcon, UserCircle, Car } from 'lucide-react';
...
export default function AdminGarageDetailsPage() {
  ...
  const handleToggle = async (action) => {
    try {
      ...
      const res = await api.post(`/admin/partners/garages/${id}/toggle-status`, { actionType: action });
      toast.dismiss(toastId);
      if (res.data.success) {
        toast.success(`Successfully applied ${action}`);
        fetchGarageDetails(); // Refresh details
      } else {
            ...
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-xl font-black text-gray-900">Active Bookings</h2>
                    </div>
                    
                    {data.activeBookings?.length > 0 ? (
                        <div className="space-y-4">
                            {data.activeBookings.map((booking) => (
                                <div key={booking.id} className="p-4 bg-gray-50 rounded-2xl border border-gray-100 flex flex-wrap items-center justify-between gap-4 hover:border-gray-200 transition-colors">
                                    <div>
                                        <p className="text-sm font-black text-gray-900">{booking.vehicleName}</p>
                                        <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">{booking.vehicleRegistration}</p>
                                    </div>
                                    <div className="flex gap-4">
                                        <div>
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Start Date</p>
                                            <p className="text-sm font-bold text-gray-800">{booking.startDate}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">End Date</p>
                                            <p className="text-sm font-bold text-gray-800">{booking.endDate}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Status</p>
                                            <span className="px-2 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-lg">{booking.status}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="text-center py-6 bg-gray-50 rounded-2xl border border-gray-100">
                            <p className="text-gray-500 text-sm font-medium">No active bookings at the moment.</p>
                        </div>
                    )}
                </div>
