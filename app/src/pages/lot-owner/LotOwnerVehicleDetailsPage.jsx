...
export default function LotOwnerVehicleDetailsPage() {
  ...
  const fetchVehicleDetails = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://gd1-grand-auto-depot-one-9ms1.onrender.com/api/lot-owner/dashboard/vehicles/${id}${queryParams}`, {
      ...
      try {
        const sRes = await fetch('hhttps://gd1-grand-auto-depot-one-9ms1.onrender.com/api/lot-manager/my-services', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const handleStopStoring = async () => {
    ...
    try {
      const token = getToken('AccessToken');
      const response = await fetch(`https://gd1-grand-auto-depot-one-9ms1.onrender.com/api/lot-owner/dashboard/vehicles/${vehicle.vehicleId}/stop-storing`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const getUpcomingService = () => {
    ...
    return services.find(s => s.status !== 'Service Completed' && s.status !== 'Completed' && s.status !== 'Cancelled' && s.status !== 'Payment');
  };
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                {vehicle.hasServiceRecommendation && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-orange-50 border border-orange-200 text-orange-600 text-xs font-bold">
                    <Wrench size={12} /> Service Recommended
                  </span>
                )}
              </div>
