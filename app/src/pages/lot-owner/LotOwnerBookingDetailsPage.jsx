...
export default function LotOwnerBookingDetailsPage() {
  ...
  const getImageUrl = (url) => {
    ...
    return `https://gd1-grand-auto-depot-one-9ms1.onrender.com${url.startsWith('/') ? '' : '/'}${url}`;
  };
  ...
  const handleVerify = async (isApproved) => {
    ...
    try {
      ...
      const res = await fetch(`https://gd1-grand-auto-depot-one-9ms1.onrender.com/api/LotBooking/${id}/verify`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: formData
      });
