...
import { 
  ...
  MapPin, Eye, Car, Shield, CheckCircle2, Loader2, Play
} from 'lucide-react';

export default function ManagerSelfDropDetailsPage() {
  const { id } = useParams();
  ...
  const fetchDetail = async () => {
    try {
      ...
      const res = await fetch(`https://gd1-grand-auto-depot-one-9ms1.onrender.com/api/lot-manager/self-drops/${id}`, {
      ...
      if (result.success) {
        ...
        navigate('/lot-manager/self-drops');
      }
    } catch (err) {
    ...
    <div className="w-full max-w-[1200px] mx-auto space-y-6 pb-12 animate-fade-in relative">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
          <button 
            onClick={() => navigate('/lot-manager/self-drops')}
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs sm:text-sm mt-0.5 hidden sm:block">View customer drop-off details and start storage</p>
          </div>
        </div>

        {detail.status === 'Confirmed' && (
          <button
            onClick={() => navigate(`/lot-manager/self-drops/${id}/start-storing`)}
            className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 shadow-sm"
          >
