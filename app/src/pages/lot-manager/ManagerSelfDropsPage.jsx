...
export default function ManagerSelfDropsPage() {
  ...
  const fetchSelfDrops = async () => {
    ...
    try {
      ...
      const [pendingRes, completedRes] = await Promise.all([
        fetch('https://gd1-grand-auto-depot-one-9ms1.onrender.com/api/lot-manager/self-drops?isCompleted=false', {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch('https://gd1-grand-auto-depot-one-9ms1.onrender.com/api/lot-manager/self-drops?isCompleted=true', {
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ]);
    ...
    <div className="w-full max-w-[1600px] mx-auto space-y-6 animate-fade-in pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage vehicles dropped off by customers</p>
        </div>
      </div>
                ...
                <div className="mt-auto">
                  <button 
                    onClick={() => {
                      ...
                      navigate(`/lot-manager/self-drops/${drop.bookingId}`)
                    }}
