...
export default function ManagerStartStoringPage() {
  const { id } = useParams();
  ...
  const [rideDescription, setRideDescription] = useState('Customer handed over vehicle. Vehicle is parked and stored in lot.');
  ...
  useEffect(() => {
    getFile(`sd-front-${id}`).then(f => f && setFrontFile(f));
    getFile(`sd-rear-${id}`).then(f => f && setRearFile(f));
    getFile(`sd-left-${id}`).then(f => f && setLeftSideFile(f));
    getFile(`sd-right-${id}`).then(f => f && setRightSideFile(f));
    getFile(`sd-interior-${id}`).then(f => f && setInteriorFile(f));
    getFile(`sd-odometer-${id}`).then(f => f && setOdometerFile(f));
    const savedDesc = sessionStorage.getItem(`sd-desc-${id}`);
    if (savedDesc) setRideDescription(savedDesc);
  }, [id]);

  useEffect(() => {
    if (frontFile) saveFile(`sd-front-${id}`, frontFile);
  }, [frontFile, id]);

  useEffect(() => {
    if (rearFile) saveFile(`sd-rear-${id}`, rearFile);
  }, [rearFile, id]);

  useEffect(() => {
    if (leftSideFile) saveFile(`sd-left-${id}`, leftSideFile);
  }, [leftSideFile, id]);

  useEffect(() => {
    if (rightSideFile) saveFile(`sd-right-${id}`, rightSideFile);
  }, [rightSideFile, id]);

  useEffect(() => {
    if (interiorFile) saveFile(`sd-interior-${id}`, interiorFile);
  }, [interiorFile, id]);

  useEffect(() => {
    if (odometerFile) saveFile(`sd-odometer-${id}`, odometerFile);
  }, [odometerFile, id]);

  useEffect(() => {
    sessionStorage.setItem(`sd-desc-${id}`, rideDescription);
  }, [rideDescription, id]);
    ...
    try {
      ...
      const res = await fetch(`https://gd1-grand-auto-depot-one-9ms1.onrender.com/api/lot-manager/self-drops/${id}/start-storing`, {
        ...
        body: JSON.stringify({
          ...
          Remarks: rideDescription
        })
      });

      const data = await res.json();
      if (data.success) {
        toast.success('Storage started successfully!');
        deleteFile(`sd-front-${id}`);
        deleteFile(`sd-rear-${id}`);
        deleteFile(`sd-left-${id}`);
        deleteFile(`sd-right-${id}`);
        deleteFile(`sd-interior-${id}`);
        deleteFile(`sd-odometer-${id}`);
        sessionStorage.removeItem(`sd-desc-${id}`);
        navigate(`/lot-manager/self-drops/${id}`);
      } else {
        toast.error(data.message || 'Failed to start storing.');
      }
    } catch (err) {
      toast.dismiss('upload');
      toast.error(err.message || 'Error starting storage process.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-6 pb-24 animate-fade-in p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-black text-gray-900 tracking-tight">Start Storing</h2>
          <p className="text-gray-500 text-sm mt-0.5">Submit condition report for self drop #{id}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 space-y-8">
        ...
        <div className="space-y-3">
          ...
          <textarea 
            ...
            placeholder="E.g. Customer handed over vehicle. Vehicle is parked and stored in lot."
        ...
        <div className="pt-4 border-t border-gray-100 flex justify-end">
          ...
          >
            {submitting ? (
              ...
              <><Check size={18} /> Submit Condition Report</>
            )}
          </button>
        </div>
      </div>
